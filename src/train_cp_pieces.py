import models
from dataset import ChessMoveDataset_pieces_it
import numpy as np
import torch
...
model = models.Att_small().to(device)
...
'''
trainset,valset = ChessMoveDataset_pieces_it(),ChessMoveDataset_pieces_it(mode='val')
...
def sum_grads(model):
  train_params = filter(lambda p: p.requires_grad and p.grad is not None, model.parameters())
