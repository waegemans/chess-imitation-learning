...
for e in range(epochs):
  if len(board.move_stack) == 4 or board.is_game_over():
    board.reset()
  move = engine.play(board, chess.engine.Limit(time=0.1), game=e).move
  ...
  loss = nn.functional.nll_loss(torch.log(predicted[action_mask==1]).unsqueeze_(0), action[action_mask==1].argmax(dim=0).unsqueeze_(0))
  loss.backward()
  optimizer.step()
  ...
  r_move = np.random.permutation(list(board.legal_moves))[0]
  board.push(r_move)
engine.quit()
